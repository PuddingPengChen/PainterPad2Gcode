...
{
    QImage loadedImage;
    QImage loadedImage_scaled;
    if (!loadedImage.load(fileName))
        return false;
    if((loadedImage.width()>800)||(loadedImage.height()>600))
    {
        float scaled = qMax(800/loadedImage.width(),600/loadedImage.height());
