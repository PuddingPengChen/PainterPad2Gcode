...
void SlicerLayer::makePolygons(OptimizedVolume* ov, bool keepNoneClosed, bool extensiveStitching)
{
    for(unsigned int startSegment=0; startSegment < segmentList.size(); startSegment++)
    {
        ...
        while(true)
        {
            ...
            Pointc p0 = segmentList[segmentIndex].end;
            poly.add(p0);
            int nextIndex = -1;
            ...
            for(unsigned int i=0;i<3;i++)
            {
                ...
                {
                    Pointc p1 = segmentList[faceToSegmentIndex[face->touching[i]]].start;
                    Pointc diff = p0 - p1;
    ...
    for(unsigned int i=0;i<openPolygonList.size();i++)
    {
        ...
        for(unsigned int j=0;j<openPolygonList.size();j++)
        {
            ...
            Pointc diff = openPolygonList[i][openPolygonList[i].size()-1] - openPolygonList[j][0];
    ...
    while(1)
    {
        ...
        for(unsigned int i=0;i<openPolygonList.size();i++)
        {
            ...
            for(unsigned int j=0;j<openPolygonList.size();j++)
            {
                ...
                if (i != j)
                {
                    Pointc diff = openPolygonList[i][openPolygonList[i].size()-1] - openPolygonList[j][openPolygonList[j].size()-1];
                    int64_t distSquared = vSize2(diff);
                    if (distSquared < bestScore)
                    {
                        bestScore = distSquared;
