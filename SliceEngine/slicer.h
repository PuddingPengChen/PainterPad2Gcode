...
private:
    gapCloserResult findPolygonGapCloser(Pointc ip0, Pointc ip1)
    {
        gapCloserResult ret;
    ...
    closePolygonResult findPolygonPointClosestTo(Pointc input)
    {
        closePolygonResult ret;
        for(unsigned int n=0; n<polygonList.size(); n++)
        {
            Pointc p0 = polygonList[n][polygonList[n].size()-1];
            for(unsigned int i=0; i<polygonList[n].size(); i++)
            {
                Pointc p1 = polygonList[n][i];
