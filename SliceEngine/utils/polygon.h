...
public:
    ...
    Pointc operator[] (unsigned int index) const
    {
    ...
    {
        int64_t length = 0;
        Pointc p0 = (*polygon)[polygon->size()-1];
    ...
    Pointc closestPointTo(Pointc p)
    {
        Pointc ret = p;
...
public:
    ...
    int64_t polygonLength() const
    {
        ...
        {
            Pointc p0 = polygons[i][polygons[i].size()-1];
            for(unsigned int n=0; n<polygons[i].size(); n++)
            {
                Pointc p1 = polygons[i][n];
                length += vSize(p0 - p1);
                p0 = p1;
            }
        }
...
public:
    ...
    {
        min = Pointc(LLONG_MAX, LLONG_MAX);
        max = Pointc(LLONG_MIN, LLONG_MIN);
