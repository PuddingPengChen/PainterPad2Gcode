...
bool SupportPolyGenerator::needSupportAt(Pointc p)
{
...
void SupportPolyGenerator::lazyFill(Pointc startPoint)
{
    static int nr = 0;
    nr++;
    ...
    while(1)
    {
        ...
        while(needSupportAt(p + Pointc(1, 0)))
        {
            p.X ++;
            done[p.X + p.Y * storage.gridWidth] = nr;
        }
        tmpPoly.add(startPoint * storage.gridScale + storage.gridOffset - Pointc(storage.gridScale/2, 0));
        ...
        while(needSupportAt(startPoint - Pointc(1, 0)) && startPoint.X > 1)
            startPoint.X --;
    }
}
...
: storage(storage), z(z), everywhere(storage.everywhere)
{
    ...
    for(int32_t y=1; y<storage.gridHeight; y++)
    {
        ...
        {
            if (!needSupportAt(Pointc(x, y)) || done[x + y * storage.gridWidth]) continue;
