...
bool Comb::preTest(Pointc startPoint, Pointc endPoint)
{
    return collisionTest(startPoint, endPoint);
}

bool Comb::collisionTest(Pointc startPoint, Pointc endPoint)
{
    Pointc diff = endPoint - startPoint;
    ...
    for(unsigned int n=0; n<boundery.size(); n++)
    {
        if (boundery[n].size() < 1)
            continue;
        Pointc p0 = matrix.apply(boundery[n][boundery[n].size()-1]);
        for(unsigned int i=0; i<boundery[n].size(); i++)
        {
            Pointc p1 = matrix.apply(boundery[n][i]);
...
Pointc Comb::getBounderyPointWithOffset(unsigned int polygonNr, unsigned int idx)
{
    Pointc p0 = boundery[polygonNr][(idx > 0) ? (idx - 1) : (boundery[polygonNr].size() - 1)];
    Pointc p1 = boundery[polygonNr][idx];
    Pointc p2 = boundery[polygonNr][(idx < (boundery[polygonNr].size() - 1)) ? (idx + 1) : (0)];
    
    Pointc off0 = crossZ(normal(p1 - p0, MM2INT(1.0)));
    Pointc off1 = crossZ(normal(p2 - p1, MM2INT(1.0)));
    Pointc n = normal(off0 + off1, MM2INT(0.2));
...
bool Comb::checkInside(Pointc p)
{
    ...
    for(unsigned int n=0; n<boundery.size(); n++)
    {
        if (boundery[n].size() < 1)
            continue;
        Pointc p0 = boundery[n][boundery[n].size()-1];
        for(unsigned int i=0; i<boundery[n].size(); i++)
        {
            Pointc p1 = boundery[n][i];
...
bool Comb::moveInside(Pointc* p, int distance)
{
    Pointc ret = *p;
    ...
    for(unsigned int n=0; n<boundery.size(); n++)
    {
        ...
        for(unsigned int i=0; i<boundery[n].size(); i++)
        {
            ...
            Pointc q = p0 + pDiff * distOnLine / lineLength;
...
bool Comb::calc(Pointc startPoint, Pointc endPoint, vector<Pointc>& combPoints)
{
    ...
    while(true)
    {
        ...
        pointList.push_back(matrix.unapply(Pointc(minX[n] - MM2INT(0.2), sp.Y)));
        ...
        pointList.push_back(matrix.unapply(Pointc(maxX[n] + MM2INT(0.2), sp.Y)));
